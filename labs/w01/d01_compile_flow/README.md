...
1. .cpp 独立编译成 .o；.o 已包含机器码，但还可能有未解析符号。
2. 链接器负责在多个 .o 和库之间解析符号、完成重定位，产出可执行文件。
3. if (x = 3) 是合法 C++：它先把 x 赋值为 3，再把 3 转成 true。
4. warning 表示“代码可构建但存在高风险语义”，新代码应尽量保持 0 warning。
